...
class Server:
    def __init__(self):
        ...
        self._clients_dict: dict = {}  # structure - client_socket: {"addr": addr, "nickname": nickname, "num_till_ban": num_till_ban)
        logging.info("=====Server is up and running=====")

        self._blocked_macs_data: None | str = None
        self._arp_cache: dict[str: str] = {}  # structure: {IP: MAC}
        self._ensure_blocked_macs_file()

    def _ensure_blocked_macs_file(self) -> None:
        """Creates blocked_macs.json file if it doesn't exist."""
                ...
                if client_msg == "BAN_ACK ":
                    ...
                    if is_contain_profanity(client_msg):
                        ...
                        if self._clients_dict[client_socket]['num_till_ban'] == 3:
                            ...
                            sleep(0.15)
                            self._broadcast(self._clients_dict[client_socket]['nick'] + " has been permanently banned from using the safe chat, behave yourself!")
                            sleep(0.15)
    ...
    def _extracts_user_mac_from_ip(self, ip: str) -> str:
        ...
        try:
            client_mac: str = subprocess.check_output(f"arp -a {ip}", shell=True)\
                .decode().split(ip)[1].lstrip().split(" ")[0]
            return client_mac
        except Exception as e:
            logging.info("Problem with finding the client's MAC address: " + str(e))
        finally:
            return ""  # consider in the check
